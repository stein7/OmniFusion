...
def pers2equi_org(pers_img, fov, nrows, patch_size, erp_size, layer_name):
    bs = pers_img.shape[0]
...
def pers2equi(pers_img, fov, nrows, patch_size, erp_size):
    bs = pers_img.shape[0]
    channel = pers_img.shape[1]
    device=pers_img.device
    height, width = pair(patch_size)
    ...
    wa = (x1.type(torch.float32)-new_x_patch) * (y1.type(torch.float32)-new_y_patch) #x0, y0
    wb = (x1.type(torch.float32)-new_x_patch) * (new_y_patch-y0.type(torch.float32)) #x0, y1
    wc = (new_x_patch-x0.type(torch.float32)) * (y1.type(torch.float32)-new_y_patch) #x1, y0
    wd = (new_x_patch-x0.type(torch.float32)) * (new_y_patch-y0.type(torch.float32)) #x1, y1
...
def pers2equi_change_num(pers_img, fov, nrows, patch_size, erp_size, pickuplist, cp_random=None):
    bs = pers_img.shape[0]
    channel = pers_img.shape[1]
    device=pers_img.device
    height, width = pair(patch_size)
    ...
    ##########------------ Random replace(important) ------------#############
    if not cp_random is None:
        print('cp random ON')
        cp = cp_random
    ...
    ### pick up some patchs
    output_a = output_a[..., pickuplist]
    output_b = output_b[..., pickuplist]
    output_c = output_c[..., pickuplist]
    output_d = output_d[..., pickuplist]
    w_list = w_list[pickuplist]
    ###
    
    w_list = w_list.permute(1, 2, 0, 3)
    w_list = w_list.flatten(2)
    w_list *= torch.gt(w_list, 1e-5).type(torch.float32)
    w_list = F.normalize(w_list, p=1, dim=-1).reshape(erp_h, erp_w, output_a.shape[-1], 4) ###instad of total n_patch
    w_list = w_list.unsqueeze(0).unsqueeze(0)
    output = output_a * w_list[..., 0] + output_b * w_list[..., 1] + \
        output_c * w_list[..., 2] + output_d * w_list[..., 3]
    img_erp = output.sum(-1) 
    
    #img_erp = output[..., pickuplist].sum(-1)  ###-> generate ghost effect due to weighted products
    return img_erp



def pers2equi_non_ideal(pers_img, fov, nrows, patch_size, erp_size, cp_random):
    bs = pers_img.shape[0]
    channel = pers_img.shape[1]
    device=pers_img.device
    height, width = pair(patch_size)
...
def pers2equi_block(pers_img, fov, nrows, patch_size, erp_size):
    bs = pers_img.shape[0]
    channel = pers_img.shape[1]
    device=pers_img.device
    height, width = pair(patch_size)
    ...
    n_patch, cp = get_centerpoints(nrows, PI, PI_2)
    
    cos_c, new_x, new_y = get_uv_coord(erp_h, erp_w, FOV, PI, PI_2, cp)
    
    w_list, mask, x0, x1, y0, y1, z = prepare_bilinear(device, height, width, erp_h, erp_w, n_patch, cos_c, new_x, new_y)
    
    img_erp = implement_bilinear(pers_img, erp_h, erp_w, n_patch, w_list, mask, x0, x1, y0, y1, z) 

    return img_erp

def implement_bilinear(pers_img, erp_h, erp_w, n_patch, w_list, mask, x0, x1, y0, y1, z):
    Ia = pers_img[:, :, y0, x0, z]
    Ib = pers_img[:, :, y1, x0, z]
    Ic = pers_img[:, :, y0, x1, z]
    Id = pers_img[:, :, y1, x1, z]
...
def prepare_bilinear(device, height, width, erp_h, erp_w, n_patch, cos_c, new_x, new_y):
    cos_c_mask = cos_c.reshape(n_patch, erp_h, erp_w)
    cos_c_mask = torch.where(cos_c_mask > 0, 1, 0)
    ...
    z = torch.arange(n_patch)
    z = z.reshape(n_patch, 1, 1)
    return w_list,mask,x0,x1,y0,y1,z

def get_uv_coord(erp_h, erp_w, FOV, PI, PI_2, cp):
    lat_grid, lon_grid = torch.meshgrid(torch.linspace(-PI_2, PI_2, erp_h), torch.linspace(-PI, PI, erp_w))
    lon_grid = lon_grid.float().reshape(1, -1)#.repeat(num_rows*num_cols, 1)
    lat_grid = lat_grid.float().reshape(1, -1)#.repeat(num_rows*num_cols, 1) 
    cos_c = torch.sin(cp[..., 1]) * torch.sin(lat_grid) + torch.cos(cp[..., 1]) * torch.cos(lat_grid) * torch.cos(lon_grid - cp[..., 0])
    ...
    new_x = new_x / FOV[0] / PI   # -1 to 1
    new_y = new_y / FOV[1] / PI_2
    return cos_c,new_x,new_y

def get_centerpoints(nrows, PI, PI_2):
    if nrows==4:    
        num_rows = 4
        num_cols = [3, 6, 6, 3]
        phi_centers = [-67.5, -22.5, 22.5, 67.5]
